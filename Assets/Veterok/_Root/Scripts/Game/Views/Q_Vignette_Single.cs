...
{
    public event Action AlphaZeroed;
    ...
    {
        Color tempColor = mainColor;
        var pulsationSpeed = 2f;
        
        while (!IsStopped)
        {
            IsPulsating = true;
            float diff = Mathf.PingPong((Time.time) * pulsationSpeed, 2f);
            float transparancy = Mathf.Lerp(.1f, 0.43f, diff);
            tempColor.a = transparancy;
            mainColor.a = tempColor.a;
